...
def obtener_nivel(x):
    x = max(x, min(25)) #25 si el peso va de 1 a 5, 15 si va de 1 a 3
...
def obtener_nivel_habilidad(id_empleado):
    habilidades = db.realizar_consulta(conexion,"query para obtener pesos de habilidades de empleado %s", (empleado_id,))
...
def obtener_nivel_certificacion(id_empleado):
    certificaciones = db.realizar_consulta(conexion,"query para obtener pesos de certificaciones de empleado %s", (empleado_id,))
...
def obtener_nivel_presencia_en_proyectos(id_empleado):
    presencias = db.realizar_consulta(conexion,"query para obtener pesos de proyectos de empleado %s", (empleado_id,))
